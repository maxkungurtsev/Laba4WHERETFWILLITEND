...
void Gdevice::CreateID3DResourse(D3D12_HEAP_PROPERTIES& heapProps, D3D12_RESOURCE_DESC& resdesc, ComPtr<ID3D12Resource> resourse, D3D12_CLEAR_VALUE* clear_value) {
...
Gdevice::Gdevice(UINT width, UINT height, int num_descriptors) {
    ...
    fence_->CreateFence(self);
    cmd_->CreateCMD(self);
    heaps_->CreateGHeaps(num_descriptors, self);
