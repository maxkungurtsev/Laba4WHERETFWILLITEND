...
Zbuffer::Zbuffer(TGAImage image, std::string& name, std::shared_ptr<Gdevice> device, TextureUsage usage) {
	...
	handle_ = device->heaps_->CreateRTV_CPU(z_buffer_->GetResourse()->GetResourse());
}
...
Zbuffer::Zbuffer(std::shared_ptr<GResourse> Gresourse, TextureUsage usage) {
	...
	handle_ = Gresourse->GetDevice()->heaps_->CreateRTV_CPU(z_buffer_->GetResourse()->GetResourse());
}
Zbuffer::Zbuffer(std::shared_ptr<GTexture> texture) {
	z_buffer_ = texture;
	handle_ = z_buffer_->GetResourse()->GetDevice()->heaps_->CreateRTV_CPU(z_buffer_->GetResourse()->GetResourse());
}