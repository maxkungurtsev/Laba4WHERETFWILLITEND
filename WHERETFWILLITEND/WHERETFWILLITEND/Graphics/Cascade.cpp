...
Cascade::Cascade(UINT width, UINT height, std::shared_ptr<Gdevice> device, XMVECTOR light_pos, XMVECTOR direction): width_(width), height_(height) {
		if (width_ == 0 || height_ == 0) {
			throw std::runtime_error("ShadowMap dimensions must be greater than zero");
		}
		std::string cascade_name = "cascade_";
		XMMATRIX view = XMMatrixLookAtLH(light_pos, direction,XMVectorSet(0, 1, 0, 0));
