...
public:
	...
	Handle CreateDSV_CPU(D3D12_DEPTH_STENCIL_VIEW_DESC dsvDesc, ComPtr<ID3D12Resource>& resourse);
	...
	Handle MakeSampler();
	Handle RewriteSRV_CPU(D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc, ComPtr<ID3D12Resource>& resourse, Handle handle);
};