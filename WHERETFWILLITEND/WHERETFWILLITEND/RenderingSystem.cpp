...
void RenderingSystem::ParticlePass() {
    ...
    auto renderBuffer = emiter_->GetAliveIn(); 
    const int renderAliveCount = renderBuffer->GetData().size();
    ...
    emiter_->GetParticleRenderCB()->Save_changes();
    const UINT aliveCount = static_cast<int>(renderBuffer->GetData().size());
    if (aliveCount == 0) {
        return;
    }
    /*
