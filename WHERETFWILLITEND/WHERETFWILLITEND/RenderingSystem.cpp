...
void RenderingSystem::ComputePass() {
    static bool firstComputePass = true;
    ...
    auto& simCB = emiter_->GetParticleSimCB()->GetData();
    ...
    if (!firstComputePass) {
        ...
        simCB.aliveInCount_ = static_cast<int>(aliveIn->GetCachedCounterValue());
        simCB.deadInCount_ = static_cast<int>(deadIn->GetCachedCounterValue());
    }

    simCB.emitCount_ = min(simCB.emitCount_, simCB.deadInCount_);
    emiter_->GetParticleSimCB()->Save_changes();
...
void RenderingSystem::RenderFrame(float time, XMVECTOR look_at, XMVECTOR cam_pos, XMVECTOR up, D3D12_CPU_DESCRIPTOR_HANDLE& rtvHandle, bool shootlight, bool culling_enabled) {
    ...
    if (emiter_ != nullptr) {
        ...
        ComputePass();
        OutputDebugStringA((std::to_string(emiter_->GetAliveIn()->GetCachedCounterValue()) + '\n').c_str());
        OutputDebugStringA((std::to_string(emiter_->GetAliveOut()->GetCachedCounterValue()) + '\n').c_str());
        OutputDebugStringA((std::to_string(emiter_->GetDeadIn()->GetCachedCounterValue()) + '\n').c_str());
        OutputDebugStringA((std::to_string(emiter_->GetDeadOut()->GetCachedCounterValue())+'\n').c_str());
        ParticlePass();
    }
